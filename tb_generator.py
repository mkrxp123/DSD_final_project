...
genData = lambda row, col, bound=(-100, 100): np.random.randint(bound[0], bound[1], size=(row, col), dtype=np.int)
...
def div(a: int, b: int, dest:int, constant: int = 0) -> np.ndarray:
    ...
    if(constant):
        return np.sign(data[a] * constant)*(abs(data[a]) // abs(constant))
    else:
...
def mod(a: int, b: int, dest:int, constant: int = 0) -> np.ndarray:
    ...
    if(constant):
        return np.sign(data[a])*(abs(data[a]) % abs(constant))
    else:
...
def matmul(a: int, b: int, dest:int, constant: int = 0) -> np.ndarray:
    ...
    if(constant):
        return data[a] @ np.full((data[a].shape[0], data[a].shape[1]), constant)
    else:
        return data[a] @ data[b]
...
for i in range(6):
    func = op[i]
    ans.append(func(random.randint(0,data_size-1), random.randint(0,data_size-1), data_size + instr_size -6 + i, random.randint(1, 10)))
